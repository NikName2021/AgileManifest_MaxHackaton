...
- Исходящие сообщения в MAX (карточки, уведомления, напоминания) отправляются напрямую, без
  собственной очереди/лимита запросов в один чат и без retry с backoff при ошибке — при сбое
  каждый вызов просто логирует ошибку (см. try/catch на местах отправки). Для MVP-нагрузки
  хакатона это осознанно принятый риск, а не забытая задача.
